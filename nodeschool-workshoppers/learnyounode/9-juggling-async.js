...
// perform HTTP GET requres to a url at specified index
// once data is received from the server, add that data to the results array at the correct location
...
var httpGetAndCheck = function(index) {
  http.get(urls[index], function(res) {
      res.pipe(bl(function(err, buf) {
        ...
        //all data recieved, add to responses array at correct location;
...
for (var i = 0; i < urls.length; i++) {
  ...
  responses.push("");
  //perform get request and check for sucess
  httpGetAndCheck(i);
}
